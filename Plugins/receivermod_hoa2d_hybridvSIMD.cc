...
class hoa2d_hybridvSIMD_t : public TASCAR::receivermod_base_t {
public:
  ...
  hoa2d_hybridvSIMD_t(tsccfg::node_t xmlsrc);
  ~hoa2d_hybridvSIMD_t();
...
hoa2d_hybridvSIMD_t::data_t::data_t(uint32_t channels )
{
...
hoa2d_hybridvSIMD_t::hoa2d_hybridvSIMD_t(tsccfg::node_t xmlsrc)
...
hoa2d_hybridvSIMD_t::~hoa2d_hybridvSIMD_t()
{
  for (int i =0;i<diviser;i++){
...
void hoa2d_hybridvSIMD_t::add_pointsource(const TASCAR::pos_t& prel, double width, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{
...
void hoa2d_hybridvSIMD_t::configure()
{
  n_channels = channels;
...
void hoa2d_hybridvSIMD_t::postproc(std::vector<TASCAR::wave_t>& output){
  ...
  for (int i =0;i<diviser;i++){
    if (vspkassign[i] ==1){
      ...
      for(uint32_t acn=0;acn<channels;++acn){
        ...
        __m128 b_vec = _mm_set_ps(B[acn],B[acn],B[acn],B[acn]);
        uint32_t t=0;
        for(;t+4<chunk_size;t+=4){
      //get [t t+1 t+2 t+3]
          __m128 t_vec = _mm_loadu_ps(&vspkchannels[i][t]);
      //calculate bxt
...
void hoa2d_hybridvSIMD_t::add_diffuse_sound_field(const TASCAR::amb1wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{
  if( output.size() ){
...
TASCAR::receivermod_base_t::data_t* hoa2d_hybridvSIMD_t::create_state_data(double srate, uint32_t fragsize) const
{
  return new data_t(channels);
}

REGISTER_RECEIVERMOD(hoa2d_hybridvSIMD_t);
